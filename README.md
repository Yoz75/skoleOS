<img width="1287" height="870" alt="изображение" src="https://github.com/user-attachments/assets/b17bc7ea-52c7-46fc-a588-aa0e31139916" />

# What is it
SkoleOS is a small UEFI application [with a crooked spine]. I'm not a pro in osdev, this project is made to proof that it is possible to make an UEFI application **using dub on Windows**.

# Dependencies
* Windows (actually it is possible to modify the dub.json file and run it on Linux)
* dub
* uefi-d
* Qemu
* OVMF is already in the repositrory!!!

# Building
* runqemu.bat -- run in debug mode
* runqemur.bat -- run in release mode
